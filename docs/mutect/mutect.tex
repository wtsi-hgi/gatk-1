...
Read orientation artifact, also known as orientation bias, arises due to a chemical change to the nucleotide during library prep that results in, for example, G base pairing with A instead of C. Such a change is very base-specific and thus occurs only on one strand of the DNA. We observe such an artifact mode as alt reads at a locus being all F1R2 or all F2R1, and this signature repeating for a particular substitution (e.g. $\text{C} \rightarrow \text{A}$) under a particular reference context (ACT) across the bam.
...
We use the EM-algorithm to learn the prior probabilities $\pi$. Say more about implementation (data collection then learning step).
